...
async function detectionLoop() {
  ...
  for (const det of detections) {
    ...
    for (const pt of lm.positions) {
      overlayCtx.beginPath();
      overlayCtx.arc(pt.x, pt.y, 1.4, 0, Math.PI*2);
      overlayCtx.fill();
    }
    ...
    const expText = `${maxExp ? maxExp[0] + ' (' + (maxExp[1]*100|0) + '%)' : '—'}`;
    expressionsEl.innerText = expText;

    // age & gender
    const ageVal = det.age ? det.age.toFixed(0) : '—';
    const genderVal = det.gender ? '${det.gender} (${(det.genderProbability*100|0)}%)' : '—';
